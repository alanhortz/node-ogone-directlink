...
Ogone.prototype.createAliasRequest = function(json) {
    json = json || {};
    Object.merge(json, this.defaults, true);
...
function AliasRequest(mode, json, algorithm, key) {
    ...
    Request.call(this, 'https://secure.ogone.com/ncol/' + mode + '/alias_gateway.asp', json, schema);
    this.algorithm = algorithm;
    this.key = key;
}

AliasRequest.prototype.__proto__ = Request.prototype;
...
AliasRequest.hashify = function(algorithm, key, obj) {
    ...
    var token = keys.sort().map(function(key) {
        if (obj[key]) return key.toUpperCase() + '=' + obj[key];
    }).join(key) + key;
    return shasum.update(token).digest('hex').toUpperCase();
};

AliasRequest.prototype.send = Request.prototype.operation.fill(undefined);
